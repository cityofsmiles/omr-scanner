...
\newcommand{\markerSize}{0.16}
\newcommand{\markerMargin}{0.18}
...
\begin{tikzpicture}[remember picture, overlay, x=1in, y=-1in, shift={(current page.north west)}]
  ...
  \foreach \gridX in {0, 1} {
    \foreach \gridY in {0, 1} {
      ...
      % 3. Header Block
      \node[anchor=north west, font=\tiny] at (\offsetX + \markerMargin + 0.1, \offsetY + \markerMargin + 0.12) {Name: \underline{\hspace{1.5in}} \quad Section: \underline{\hspace{0.7in}}};
      \node[anchor=north west, font=\tiny] at (\offsetX + \markerMargin + 0.1, \offsetY + \markerMargin + 0.42) {Test: \underline{\hspace{1.5in}} \quad Date: \underline{\hspace{0.7in}}};
